...
const myRoadmap = async (request, reply) => {
    ...
    data.sections = quickSort(await Promise.all(sectionDatas.map(async (section) => {
        const detailDatas = await Detail.find({ section_id: section._id });
        const result = {
            section_id: section._id,
            section_name: section.section_name,
            order: section.order,
            details: await Promise.all(detailDatas.map(async (detail) => {
                totalDetail++;
                return {
...
export { getCoursesSimilarTag, myRoadmap };
