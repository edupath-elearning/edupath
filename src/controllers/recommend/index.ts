...
import { handleRecommend } from '~python/recommend';
import { ERecommendStatus } from '~types/index';

export const checkStatus = async (request: FastifyRequest<{ Params: { user_id: string } }>, reply: FastifyReply) => {
  ...
  const result = {
    ...
    prediction: await (async () => {
      return await Promise.all(
        userData!.recommend!.map(async (recommendItem) => {
...
export const listTag = async (request: FastifyRequest, reply: FastifyReply) => {
  const tags = await Tag.find({}, { tag_name: 1, _id: 1 });
  const result = tags.map((x) => x.tag_name);
  await reply.code(200).send(result);
};

export const processRecommend = async (
  request: FastifyRequest<{ Body: { skills: string[] } }>,
  reply: FastifyReply,
) => {
  ...
  const setRecommend = async () => {
    const maintypes = await Promise.all(
      prediction.map(async (type, index) => {
...
export const chooseMaintype = async (
  request: FastifyRequest<{ Querystring: { choosen: string } }>,
  reply: FastifyReply,
) => {
